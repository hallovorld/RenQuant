...
# Defaults matching US federal + state combined rates
DEFAULT_TAX_CONFIG = {
...
def load_tax_config(strategy_config: dict) -> dict:
    """Extract the ``tax`` block from a strategy config, with defaults."""
...
def tax_rate_for_holding(holding_days: int, short_term_rate: float,
                         long_term_rate: float, long_term_threshold_days: int) -> float:
    """Return the applicable tax rate based on holding period."""
    if holding_days >= long_term_threshold_days:
        return long_term_rate
    return short_term_rate


def compute_trade_tax(pnl: float, holding_days: int, short_term_rate: float = 0.50,
                      long_term_rate: float = 0.32,
                      long_term_threshold_days: int = 365) -> float:
    """Compute tax owed on a single trade.

    Returns 0.0 for losses (no tax on losses; loss harvesting is not modeled).
    """
    if pnl <= 0:
        return 0.0
...
def compute_after_tax_pnl(pnl: float, holding_days: int, short_term_rate: float = 0.50,
                          long_term_rate: float = 0.32,
                          long_term_threshold_days: int = 365) -> float:
    """Return after-tax P&L for a single trade."""
...
def add_tax_columns(trades_df: pd.DataFrame, tax_config: dict) -> pd.DataFrame:
    """Add tax-related columns to a closed-trades DataFrame.

    Expects columns: ``entry_time``, ``exit_time``, ``pnl``.
    Adds: ``holding_days``, ``tax_rate``, ``tax``, ``after_tax_pnl``.
    """
    df = trades_df.copy()
    if df.empty:
        df["holding_days"] = []
