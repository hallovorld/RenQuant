"""S-FRAC stage 3 (core, sprint D2) — software-stop layer wiring tests.
...
(``SoftwareStopRegistry``/``compute_staleness``/etc.) moved to
``renquant_pipeline.software_stops`` (renquant-pipeline#167) — new
capability logic belongs in an owning repo, not the umbrella
(RenQuant#440 review, 4 rounds). This file now carries ONLY the
umbrella-specific orchestration-wiring coverage that can't move (it
exercises the real ``RunnerAdapter``/``FakeBroker``/ops-watchdog-script,
all umbrella-only):

  1. Flag-off byte-inertness on the sell-only loop  -> TestFlagOffInert
  2. Stage-0 capability-gate integration (a corrupt
     registry blocks a REAL RunnerAdapter.commit)   -> TestCorruptRegistryFailClosed
  3. Ops watchdog CLI script's own exit codes       -> TestStalenessWatchdog
  4. Commit-path wiring through the stage-0 seam
...
The registry's own unit-test coverage (round-trip, ratchet-only invariant,
trigger correctness, gap-through pricing, corruption schema-validation,
staleness arithmetic) now lives in renquant-pipeline's own
``tests/test_software_stops.py``.
"""
...
class TestCorruptRegistryFailClosed:
    """Stage-0 capability-gate integration only — a corrupt registry
    blocking a REAL RunnerAdapter.commit. The registry's own pure
    corruption-detection unit tests (test_corrupt_is_not_armed,
    test_schema_violation_is_corrupt, test_corrupt_refuses_writes_and_
    preserves_bytes, test_corrupt_watchdog_state) moved to
    renquant-pipeline's own test_software_stops.py."""
...
class TestStalenessWatchdog:
    """Ops watchdog CLI script's own exit codes only — the pure
    compute_staleness arithmetic and registry heartbeat-stamping unit
    tests (test_arithmetic, test_default_budget_matches_loop_cadence,
    test_evaluate_stamps_fresh_heartbeat) moved to renquant-pipeline's
    own test_software_stops.py."""
