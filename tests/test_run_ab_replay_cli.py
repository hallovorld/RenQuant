...
from kernel.portfolio_qp.allocator_replay import AllocatorReplayBar  # noqa: E402
...
class TestRegistry:
    ...
    def test_step4_allocators_registered(self):
        ...
        from kernel.portfolio_qp.baseline_allocators import (
...
class TestRegimeStratifiedBlock:
    def test_per_regime_n_bars_correct(self):
        ...
        from kernel.portfolio_qp.allocator_replay import replay_all
...
class TestCLISmoke:
    ...
    def test_main_loader_module_receives_fwd_horizon_kwarg(self, tmp_path):
        ...
        (loader_dir / "loader.py").write_text(
            "import sys\nfrom pathlib import Path\n"
            "BACKTESTING_104 = Path(__file__).resolve().parents[5] / "
            "'backtesting' / 'renquant_104'\n"
            "if str(BACKTESTING_104) not in sys.path:\n"
            "    sys.path.insert(0, str(BACKTESTING_104))\n"
            "import numpy as np\n"
            "from kernel.portfolio_qp.allocator_replay import AllocatorReplayBar\n"
...
class TestZeroBarsGuard:
    def test_zero_bars_emits_invalid_experiment_not_crash(self, tmp_path):
        ...
        not a crash deep in np.max() on an empty array."""
        ...
        try:
            ...
            assert rc == 2  # non-zero: experiment invalid
            payload = json.loads(out.read_text())
