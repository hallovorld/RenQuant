...
class TestLTREarlyStopping:
    def test_train_kwargs_branched_on_eval_data(self):
        ...
        # 2026-04-26 (X1+X2 completed): Python-level early stopping via
        # incremental training + spearman IC. Marker: chunked while loop.
        assert ("early_stopping_rounds=int(early_stopping_rounds)" in src
                or 'X1+X2 (2026-04-26, completed)' in src
                or 'while rounds_done < num_boost_round' in src)
