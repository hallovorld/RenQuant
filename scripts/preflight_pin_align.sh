...
# local sibling checkouts matched subrepos.lock.json (the umbrella was a day
# behind origin/main, so the run used yesterday's pins). Policy (operator
# go-ahead): auto-align each sibling checkout to its PINNED commit, then trade;
# ABORT if a repo is dirty or its pin is unreachable — never trade unaudited
# code.
#
# `subrepo_assemble.py --sync` checks out a clean-but-drifted repo to its lock
# commit and refuses (exit 1) on a dirty repo; `--dry-run` skips writing the
# assembly bundle. When already pinned it is a fast no-op (no fetch/checkout).
#
...
if [ "${RENQUANT_SKIP_PIN_SYNC:-0}" != "1" ]; then
    ...
    echo "Aligning subrepo checkouts to pinned commits (subrepos.lock.json)…"
    if ! "$PYTHON" "$REPO_DIR/scripts/subrepo_assemble.py" --sync --dry-run; then
