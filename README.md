...
Four regimes with distinct parameters: `BULL_CALM` (momentum entry, 15% max position, 15% stop, 20% trailing-stop trigger / 18% trail, `max_hold_days=500`), `BULL_VOLATILE` (capitulation entry on high-volume down-close, 20% max, 5% stop, `max_hold_days=500`), `CHOPPY` (divergence-from-SPY entry, 15% max, 5% stop, `max_hold_days=23`), `BEAR` (all offensive buys blocked; up to 1 defensive position in GLD/TLT/XLV/XLU at 15% of portfolio; existing positions held until stop-loss or sell signal). Entry gates: **SPY EMA50 trend gate** blocks all new buys when SPY is below its 50-day EMA; **SPY velocity crash filter** blocks if SPY fell >3% in last 3 days. Stock selection pipeline: earnings filter (±3 days) → regime-conditional scan → calibrated `rank_score` filter → relative-strength score vs sector ETF → config-driven blend of normalized `rank_score` and RS → tiered `rank_score` thresholds → wash-sale recheck → sector guard (max 3 per sector) → correlation-aware greedy selection (threshold 0.70). Position sizing scales both `max_position_pct` and `cash_reserve_pct` by regime confidence before each entry is capped by the scaled position budget. Watchlist (24 symbols): equity names minus ARKK/SHOP/COIN, plus GLD, TLT, XLV, XLU as counter-cyclical defensives. Classification model trained with relative-close prices (`stock / SPY × 100`) so labels measure outperformance vs SPY rather than raw returns — prevents the bull-market always-buy bias. OOS Sharpe floor: 0.8 (matching renquant_102). `min_hold_days: 20` and 3-consecutive-sell-signal requirement prevent noise exits and extend avg hold to 150-200 days, improving long-term tax treatment. Training: fixed cutoff 2024-01-01 for stable OOS simulation; live models retrained on last 4 years and re-exported daily. Q-Learning models use a deterministic per-ticker seed (`abs(hash(ticker)) % 2^32`) so daily retraining produces stable, reproducible model selections. **Active daily strategy** — replaces renquant_102 as the live trading engine, scheduled via `scripts/daily_103.sh`. Unit tests: 462 tests in `tests/` covering the strategy, alignment, and replay-parity suites (`python -m pytest tests/ -v`).
