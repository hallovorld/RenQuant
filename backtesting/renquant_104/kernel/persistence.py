...
-- Mirror of renquant-pipeline kernel/persistence.py (#124). One
-- APPEND-ONLY row per gate verdict submission.
...
-- L6 audit sidecar persistence (umbrella mirror of renquant_pipeline). Append-
-- only score-drift history + the alert escalation book that survives restarts.
...
-- Alert incident book (eng plan §12.3 / L6 alert lifecycle). Persists the
-- escalation state machine across restarts: WITHOUT this, a daily restart
-- resets every incident to NEW and re-escalates — the noise the lifecycle
-- exists to prevent. PK = the dedup key.
...
-- Broker-reconciliation action log (eng plan §III.4; umbrella mirror of
-- renquant_pipeline). One APPEND-ONLY row per non-OK reconcile() action so
-- "when did a position vanish externally / get quarantined / force-covered"
-- is a query. OK actions (steady state) are not logged. Completes the L6
-- audit-persistence trio (gate_verdicts + score_drift_audits + this).
...
def record_gate_verdicts(
    ...
    *,
    ...
    Mirror of renquant-pipeline kernel/persistence.record_gate_verdicts
    (#124); duck-typed on registry.ledger_rows. No-op when persistence
    is disabled, run_id is missing, registry absent, or no verdicts.
    """
...
# ── L6 audit sidecar recorders (umbrella mirror of renquant_pipeline) ────────
...
def load_alert_book(conn: sqlite3.Connection | None,
    ...
    from kernel.alert_lifecycle import Alert, AlertBook  # noqa: PLC0415
...
def record_reconciliation_actions(
    ...
    *,
    ...
    reconciliation_actions (eng plan §III.4; umbrella mirror of
    renquant_pipeline).

    ``actions`` is the list from kernel.broker_reconciliation.reconcile()
    (duck-typed on .kind/.ticker/.detail/.state_qty/.broker_qty). OK actions
    are skipped (steady state). No-op when persistence is disabled, run_id is
    missing, or there are no non-OK actions. Append-only by construction.
    """
