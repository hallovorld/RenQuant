...
def _submit_gate_verdict(ctx, *, gate: str, reason: str, inputs: dict) -> None:
    """Dual-write a block verdict to the GateRegistry (eng plan S2-PR4).

    Mirror of job_panel_scoring._submit_gate_verdict: lazy import + loud
    degrade because the sibling renquant-pipeline checkout may be pinned
    before kernel.gate_registry exists (merged-not-deployed window);
    telemetry must never block trading. Direct buy_blocked writes stay
