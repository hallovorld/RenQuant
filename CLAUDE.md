...
**Adaptive regime multi-stock** (renquant_103): extends 102 with a 3-layer regime detector + cross-sectional rotation. Notebook: `Notebooks/renquant_103.ipynb`. Watchlist (24) adds GLD/TLT/XLV/XLU as defensive counter-cyclicals. Earnings calendar refreshed via `python scripts/fetch_earnings_calendar.py --strategy renquant_103`. New artifacts (under `artifacts/`): `spy-gmm-regime.json`, `watchlist-correlation.json`, `earnings-calendar.json`. DataJob checks `artifacts/` first then strategy root.

*Regime detector:*
- Layer 1: rolling Hurst (63d window, `hurst_trending_threshold=0.65`, `hurst_reversion_threshold=0.52`).
- Layer 2: CUSUM changepoint → uncertainty window after each break.
- Layer 3: GMM on 4 SPY features (10d return, 20d realised vol, ADX, return autocorr) → continuous P(regime) for sizing.
- 4 regimes: BULL_CALM (momentum entry, 15% max position), BULL_VOLATILE (capitulation entry on high-vol down-close, 20%), CHOPPY (divergence — stock > SPY, 15%), BEAR (offensive buys blocked; 1 defensive slot at 15%).
- BEAR hard override: `spy_20d_ann_vol > 0.35` or `spy_20d_cumret < -0.08` forces BEAR regardless of GMM (configurable via `bear_vol_threshold` / `bear_return_threshold`) — GMM reacts too slowly to macro shocks like tariff events.
- CHOPPY confidence uses Hurst distance from reversion threshold `(hurst_rev−H)/(hurst_rev−choppy_hurst_floor)`, floor=0.20; BULL/BEAR still use GMM posterior. GMM's two clusters have ~48/48% weights so it can't produce meaningful CHOPPY confidence.

*Selection pipeline:* earnings filter (±3d) → SPY EMA50 trend gate (blocks new buys if SPY < 50d EMA) → SPY velocity crash filter (blocks if SPY down >3% in 3d) → transition uncertainty window (no buys for 3 bars after CUSUM changepoint) → BEAR branch (defensives only) → model buy signal → calibrated `rank_score` filter (0.10 BULL_CALM, 0.15 others) → RS score vs sector ETF → blended rank (config-driven `ranking.blend_weights`, refreshed daily by `scripts/recalibrate_scores.py`) → tiered thresholds (slot 1: 0.10, slot 2: 0.30, slot 3: 0.50; identical slot logic everywhere via `tier_idx = min(slots_filled, len(tiers) - 1)`) → wash-sale guard (re-checked in selection loop, not just scan) → sector guard (max 3/sector) → correlation guard (greedy, threshold 0.70).

*Sizing & holds:*
- 8 concurrent positions; `max_position_pct` and `cash_reserve_pct` scaled by regime confidence before sizing as `min(cash − reserve, portfolio × scaled_max_pct)`.
- Oversize fallback: if `max_position_pct × confidence` can't cover 1 share, retry at 25% cap (prevents silent skip on high-priced stocks like LLY).
- `min_hold_days: 30` — model-sell blocked before 30d (stop-loss + single-day-loss gate fire immediately).
- `max_hold_days`: 500 for BULL_CALM/BULL_VOLATILE/BEAR, 40 for CHOPPY (raised from 23 to fit min_hold + 3-day sell streak).
- 3 consecutive daily sell signals required before model exit.
- BULL_CALM trailing stop: 20% gain trigger, 18% trail below HWM (peak-gain based, stays armed after pullbacks).

*Exit priority order* (LEAN/notebook/live identical): trailing stop → cumulative stop-loss (15% BULL_CALM, 5% others) → single-day loss gate (`max_single_day_loss_pct: 0.10` BULL_CALM, disabled others — protects against gap-down days) → max hold → model sell streak.

*Rotation* (`RotationJob` between `RankingJob` and `SelectionJob`, skipped in BEAR): held positions compete with candidates on `rank_score` via `kernel.rotation.find_rotation_pairs()`. Tax-adjusted `effective_swap_margin = base_margin + tax_drag(unrealized_pnl, hold_days, ST/LT rate)`; positions within `lt_protection_days` of the LT threshold on a gain are pinned (+inf) to preserve the LT discount. Each pair is re-validated against wash-sale/sector/correlation guards on the virtual post-swap holdings, then emits `ExitSignal(exit_type="rotation")` + sized buy. Notebook mirrors via the same primitive. Config block: `rotation.{enabled, swap_margin, min_rotation_hold_days, lt_protection_days, max_rotations_per_bar}`.

*Models & calibration:*
- Tournament trains Classification + QLearning + Manual + XGBoost per ticker, exports best by OOS Sharpe (floor 1.0).
- Classification model gets `close = stock_close / spy_close × 100` so labels become relative outperformance vs SPY (prevents bull-market always-buy bias).
- All three trainable models use `abs(hash(ticker)) % 2^32` as a per-ticker seed.
- `predict_bulk()` returns string Series — map with `{"buy": 1, "hold": 0, "sell": -1}` for numeric use.
- `predict_score_bulk()` returns native continuous floats (not cross-model comparable). Notebook export, LEAN, and live runner calibrate via `common.models.scoring` into a common `rank_score` (P(outperform SPY by threshold% in lookahead days)) using metadata `score_calibration` (with runtime fallback). Calibration method by sample size: isotonic n≥300, Platt 120≤n<300, constant base-rate n<120. Refreshed daily by `scripts/recalibrate_scores.py`.
- Blend weights computed by the same script via logistic regression on `[norm(rank_score), norm(rs_score)]`, positive coefficients normalised into `ranking.blend_weights`.
- Live models retrained on last 4 years; fixed OOS cutoff 2024-01-01 for stable simulation.

*Live-runner specifics:*
- Per-model `feature_columns` come from policy-metadata.json (not global config — regime-context columns only exist during notebook training).
- Fetches sector ETFs (XLK, XLI, etc.) in addition to the watchlist so RS scores cover all sectors. Notebook fetches WATCHLIST ∪ sector_etf_map.values() ∪ {SPY}.
- OHLCV freshness guard: force-refreshes parquet cache via yfinance if last bar >5 trading days old.
- Duplicate-order guard: checks Alpaca open orders before any BUY; skips if pending exists.

*Test coverage:* notebook/LEAN/live aligned on calibrated `rank_score`, config-driven blend weights, tiered thresholds, wash-sale rechecks, confidence-scaled sizing. Backed by `tests/test_runner_ranking.py` (rank scoring + mixed-model ordering + score logging + tier application + blend estimation), `tests/test_strategy_ledger_parity.py` (synthetic multi-day buy ledger), and the alignment tests listed below.
...
- `training.model_ttl_days` — per-ticker gate inside `_run_ticker_chain`. When > 0, tickers whose `models/{TICKER}/{TICKER}-policy-metadata.json` has a `trained_date` within TTL are skipped (cached model reused; `tc.ttl_skipped=True`). Complements — does not replace — `training.cadence`: cadence controls whether `FullTrainingPipeline.run` executes at all; TTL then filters individual tickers within that run. `scripts/train_104.py --force` bypasses both gates. **Interaction with 104's Tue/Thu/Sun cadence**: TTL=1 is effectively idempotency (skip if already trained today — protects against daily_104 retry storms); TTL=3 → Thu skips Tue's models (Δ=2≤3), Sun retrains (Δ=5>3) → each ticker retrained ~every 5 calendar days; TTL=7 → each ticker retrained ~weekly. **Current: 1.** Change in `strategy_config.json` + `strategy_config.golden.json` together or the drift guard flags it.
