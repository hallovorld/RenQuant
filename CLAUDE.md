...
**Adaptive regime multi-stock** (renquant_103): Extends 102 with a 3-layer regime detector. Layer 1: rolling Hurst exponent (63-day, characterises momentum vs mean-reversion). Layer 2: CUSUM changepoint detection (fast transition trigger, uncertainty window after each break). Layer 3: GMM clustering on 4 SPY features (10d return, 20d realised vol, ADX, return autocorr) — outputs continuous P(regime) to scale position size. Four regimes: BULL_CALM (momentum entry, 30% max position), BULL_VOLATILE (capitulation entry on high-vol down-close, 20%), CHOPPY (divergence entry — stock outperforming SPY, 15%), BEAR (no new buys). Stock selection pipeline: earnings filter (±3d) → regime-conditional volume scan → relative-strength ranking vs sector ETF → per-symbol model as continuous scorer → combined rank (50% RS + 50% model) → correlation-aware greedy selection (threshold 0.70). New artifacts: `spy-gmm-regime.json`, `watchlist-correlation.json`, `earnings-calendar.json`. Watchlist (24): adds GLD, TLT, XLV, XLU as defensive counter-cyclical tickers; removes ARKK, SHOP, COIN. Notebook: `Notebooks/renquant_103.ipynb`. Earnings calendar refreshed via `python scripts/fetch_earnings_calendar.py --strategy renquant_103`. Full design spec: `docs/renquant_103_design.md`. Key implementation details: (1) Classification model receives `close = stock_close / spy_close × 100` so forward-return labels become relative outperformance vs SPY (prevents bull-market always-buy bias); (2) `predict_bulk()` returns string Series — map with `{"buy": 1, "hold": 0, "sell": -1}` for numeric use; (3) OOS Sharpe floor is 0.8 (matching 102) — marginal models below 0.8 are excluded; (4) `max_hold_days` is 500 for BULL_CALM, BULL_VOLATILE, BEAR (matches 102), 10 for CHOPPY; (5) position sizing uses `min(cash, portfolio × max_pos_pct)` — each position capped at 30% of total portfolio, limited by available cash; (6) `min_hold_days: 20` — positions cannot be sold via model signal until held 20 calendar days (stop-loss still triggers immediately); (7) simulation requires 3 consecutive daily sell signals before exiting a position, eliminating one-day noise flips that would generate short-term taxable events; (8) Q-Learning models use `abs(hash(ticker)) % 2^32` as a per-ticker seed before training — ensures reproducible model exports across daily retraining runs; (9) live runner uses per-model `feature_columns` (from policy-metadata.json) not the global strategy_config `feature_columns`, because regime-context columns (`spy_realized_vol`, `hurst_proxy`, etc.) exist only during notebook training, not at live inference time.
