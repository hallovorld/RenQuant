...
- **renquant_103 pipeline**: `run_once_multi()` dispatches to `_run_once_multi_pipeline()` for kernel-based strategies, which constructs a `PipelineContext` and runs `Pipeline([DataJob(), SignalJob(), ExecutionJob()])`. OHLCV fetch (DataJob) and candidate scoring (SignalJob) run in parallel via `ThreadPoolExecutor`; sell/buy phases are sequential. The legacy 900-line path is retained for renquant_102.
...
`backtesting/renquant_103/pipeline/` — 3-job execution pipeline shared by the live runner. Can import from both `kernel/` and `common/` (runs on host, not in LEAN Docker).

| Module | Contents |
|--------|----------|
| `pipeline/context.py` | `PipelineContext` — dataclass carrying all shared state between jobs (~30 fields) |
| `pipeline/task.py` | `TaskResult`, `run_tasks(tasks, max_workers=8)` — parallel execution via `ThreadPoolExecutor` |
| `pipeline/pipeline.py` | `Job` ABC (`.run(ctx)`, `.should_skip(ctx)`), `Pipeline` sequential orchestrator |
| `pipeline/jobs/data.py` | `DataJob` — parallel OHLCV fetch + artifact loading (GMM, correlation, earnings) |
| `pipeline/jobs/signals.py` | `SignalJob` — regime detection, broker account state, broker reconciliation, parallel candidate scoring |
| `pipeline/jobs/execution.py` | `ExecutionJob` — sell phase (all 5 exits via `compute_exits()`), buy phase (selection loop + orders) |
...
- Total test count as of last update: 588 collected tests. Run `python -m pytest tests/ -v` to verify.
...
| `tests/test_kernel_units.py` | 113 unit tests for all 9 kernel modules (includes market_gates, portfolio, compute_relative_strength) |
