...
| R | Diagnose stuck `TransitionWindowTask` / CUSUM buy-block | 🟠 HIGH (blocks live trading) | 2 h | 2026-04-22 + 04-23: 12 consecutive bars of `transition=True`, 0 bars of `transition=False`. Either SPY CUSUM is legitimately firing every bar (verify with SPY vol data) or `state.countdown` isn't being persisted correctly across bars. If legit, relax `transition_uncertainty_bars` from 3 → 1; if bug, fix countdown persistence. All non-rotation BUYs blocked while True. |
