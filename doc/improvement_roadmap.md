...
| ~~C~~ | ~~σ-penalty λ sweep: λ ∈ {0.1, 0.25, 0.5}~~ | ❌ shelved — `0c80443` | — | λ=0.25 wins by +2 APY pts (below +3 promotion floor). λ≥0.5 starves the ranker (53d max streak at λ=1.0). Keep additive as golden. Log: `doc/panel_training_runs.md`. |
| ~~D~~ | ~~Sustainability watch on T4 golden — 30-day live APY tracker~~ | ✅ done — `67e95af` | 2 h | daily_104.sh appends audit JSONL; `scripts/weekly_apy_check.py` computes rolling 30d APY + drawdown-streak, ntfy alert below 25% / 20% / 5d. Launchd plist loaded Sun 12 PT. Smoke-tested 4 cases. |
...
| F | Item 6 — regime-conditional calibration | 🟡 infra shipped | 2 days | Code (`fit_regime_conditional` + `LoadGlobalCalibrationTask` + per-regime dispatch in `ApplyGlobalCalibrationTask` + CLI flag `--regime-conditional` + 10 regression tests) is in. Artifact generation + A/B still pending — run `python scripts/fit_panel_calibrator.py --regime-conditional`, flip `panel_ltr.global_calibration.regime_conditional.enabled: true`, A/B vs pooled. |
| G | Item 8 — hourly-bar panel features (keep as eventual, not deferred) | 🟡 steps 1+2+3 shipped | 2-3 days | Step 1 (aggregator + 17 tests) commit `8c65537`. Step 2 (wiring into PanelDataJob + 8 tests) commit `f03d1eb`. Step 3 (`scripts/fetch_hourly_bars.py` + dry-run verified) commit `0c80443`. Remaining: run the fetcher, flip `panel_ltr.hourly.enabled: true`, retrain, A/B vs daily-only golden. |
