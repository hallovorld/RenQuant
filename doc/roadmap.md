...
|---|------|---|---:|---|
| ~~**T2-1**~~ | ~~LightGBM LTR replacement~~ — **REJECTED 2026-04-27** based on S2 retrain. On the **current panel** (99-ticker, 28-feature, 753-date, no-macro), LightGBM scores OOS IC **+0.0193** vs XGBoost prod **+0.0482** (-60%). The historical "+128% IC" claim from the 491-date hourly-era panel does NOT transfer. With macro added, LGBM gets even worse: 0.0224 (-53%). Hypothesis: LGBM `lambdarank` objective and tree-growth heuristics are tuned for larger datasets (LightGBM's sweet spot is 100K+ rows; our panel is ~75K). Artifact preserved at `panel-ltr.lgbm-no-macro.bak.json` for forensics. | rejected | n/a | ❌ |
