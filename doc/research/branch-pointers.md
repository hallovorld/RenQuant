...
- ✅ **B2 sim on wl178_v2_l1sub_l2 artifact** (with Sharpe instrumentation, sim-adapter fix landed): APY +10.33%, **Sharpe +0.609, Sortino +0.527, Calmar +0.599, MaxDD 17.26%, Ann Vol 19.23%**, total return +13.83%, 71% win rate, 375 buys / 389 sells, longest no-trade streak 38d. NOT a comparable A/B (wl103 baseline B2 produced 0 trades on prior run; Sharpe target per CLAUDE.md golden goal is 2.0 — this 0.609 is ~30% of target, NOT shippable but confirms architecture works end-to-end). First sim hit a 4-tuple/3-tuple unpacking bug from cherry-pick of c679600 — produced 0 trades silently; fixed in `ee926ea` with regression test pinning the contract.
- ✅ **wl103+L1+L2 falsification A/B** — CPCV mean_ic = **+0.0328** vs baseline wl103 +0.0418 = **−21.5%**. Lands in predicted band (+0.030~+0.035). Confirms L1+L2 is universe-conditional, not universal. DB run_id `20260502095403-panel-ltr-eef109`. Detail: `failed-experiments-log.md::E_LAYER1_PLUS_2_ON_WL103`.
- ✅ **Sharpe / Sortino / Calmar / MaxDD / Vol instrumentation** cherry-picked from main (`fa7e4b7`) + sim-adapter unpacking fix (`ee926ea`). 28 risk-metric tests + 2 regression tests green.
- ✅ **Track D — watchlist quality curve design** (`doc/research/watchlist-quality-curve.md`) — three-stage admission funnel: liquidity/age mechanical → KS distributional → greedy IC-additive batch. Implementation deferred until insider/PEAD/microstructure features land first.
- ✅ **Track F — triple-barrier label design** (`doc/research/triple-barrier-label-design.md`) — Lopez de Prado AFML §3 label, A/A plan vs fwd_5d, predicted +5-10 bp lift. Implementation deferred to after richest-feature-set is ready.
- 🟡 **Track A — insider trades cold backfill** (in progress as of 2026-05-02 09:10 PT) — initial 240s budget caused 0/103 fetches (timeout); enlarged to 90 min budget with 90s/ticker. SEC fetch path proves working (single AAPL fetch in 1.3s with proper UA), but `call_with_timeout` wrapper introduces overhead causing per-ticker hits to 90s timeout. Existing 45/103 cached coverage from prior fetches is enough to validate the feature plumbing path end-to-end. Production launchd plist update for `RENQUANT_SEC_UA` env var pending operator review (per CLAUDE.md §5.5).
