...
- ✅ **B2 sim on wl178_v2_l1sub_l2 artifact** (with sim-adapter fix) — APY +10.33%, total return +13.83%, 71% win rate, 375 buys / 389 sells, longest no-trade streak 38d. NOT a comparable A/B (wl103 baseline B2 produced 0 trades on prior run). First sim with Sharpe instrumentation on this branch hit a 4-tuple/3-tuple unpacking bug from cherry-pick of c679600 — produced 0 trades silently; fixed in `ee926ea` with regression test pinning the contract.
