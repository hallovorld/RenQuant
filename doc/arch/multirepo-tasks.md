...
7. **Never interleave artifact stamping with a running gate.** Race condition: gate loads artifact at launch into memory, then writes back at exit. Any artifact mutation in between (stamping fields, manual edits, other scripts) is silently overwritten when the gate writes its in-memory copy at exit. Wait for gate completion BEFORE stamping; or stamp first THEN launch gate.
8. **`build_wf_config_from_prod` strips fields not in its whitelist.** Any new operator-controlled config block (e.g. `wf_gate` with relax flags) must be explicitly added to the propagation list in `wf_config_builder.py` or it silently disappears in `--derive-config-from-prod` runs. Regression test in `tests/test_wf_config_builder_propagation.py`.
