...
(`.github/workflows/strategy-104-snapshot-fresh.yml`). That file states a
**current fact** ("the pinned config says X, as of the last regeneration") —
it never carries a historical/promotion claim ("active since <date>",
"promoted on <date>"); that kind of dated narrative belongs HERE instead,
below, with its own explicit date. The table below is a point-in-time
narrative snapshot (dated in its own heading) — for anything it covers that
also appears in the generated file, trust the generated file for the
current-state fact, and this table only for the historical/promotion story
around it.
