...
WHAT:      (1) `scripts/weekly_wf_promote.sh --promote-staged` now calls
           `.subrepo_runtime/repos/renquant-orchestrator/ops/renquant104/a4t1_promote_staged.sh`
           (identify → atomic consume → stamp, exit 0 iff PROMOTED) in place of
           `python -m renquant_backtesting.wf_gate.freshness_fallback --stamp`,
           and adds `renquant-orchestrator` to the pinned PYTHONPATH list.
